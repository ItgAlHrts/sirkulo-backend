 ...
 * Generates clean Canvas / SVG QR Codes offline with zero dependencies
 */
(function(root, factory) {
    ...
    QR8bitByte.prototype = {
        getLength: function() { return encodeURI(this.data).replace(/%[0-9a-fA-F]{2}/g, 'a').length; },
        write: function(buffer) {
            for (var i = 0; i < this.data.length; i++) {
                var c = this.data.charCodeAt(i);
                if (c < 128) buffer.put(c, 8);
                else {
                    var s = encodeURI(this.data.charAt(i)).split('%');
                    for (var j = 1; j < s.length; j++) buffer.put(parseInt(s[j], 16), 8);
                }
            }
        }
    };
    ...
    QRCode.prototype.makeCode = function(text) {
        ...
        // Build SVG String
        var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + count + ' ' + count + '" width="100%" height="100%" style="display:block;shape-rendering:crispEdges;">';
        svg += '<rect width="' + count + '" height="' + count + '" fill="' + this.options.colorLight + '"/>';
        var path = '';
        ...
        svg += '<path d="' + path + '" fill="' + this.options.colorDark + '"/>';
        svg += '</svg>';
